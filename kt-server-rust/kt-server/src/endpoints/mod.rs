...
#[get("/")]
fn server_info(config: &State<KTConfig>, state: &State<ServerState>) -> Json<Info> {
...
pub fn build_rocket(server_state: ServerState, config: KTConfig) -> rocket::Rocket<rocket::Build> {
    info!(
        "Starting kt-server-rust at {}:{}",
...
mod tests {
    ...
    fn test_filesystem_resolver() {
        ...
        assert!(String::from_utf8(content).unwrap().contains("kt-server"));
    }
