...
if [ "$(lsof -i:$PORT -sTCP:LISTEN -t)" ]; then
    echo "KomuTracker server already running on port $PORT, using that."
else
...
sleep 1;
SYNCROOTDIR="$HOME/KomuTrackerSync"
...
function sync_host() {
    ...
    for db in $dbs; do
        ...
        BUCKETS="kt-watcher-window_$host,kt-watcher-afk_$host"

        echo "Syncing $db to $host"
        cargo run --bin kt-sync -- $AWSYNC_ARGS sync-advanced $AWSYNC_ARGS_ADV --mode pull --buckets $BUCKETS
...
if [ -z "$host" ]; then
    ...
    sleep 0.5
    # For each host in the sync directory, pull the data from each database file using kt-sync
...
# kill kt-server-rust (if started by us)
if [ "$SERVER_PID" ]; then
    kill $SERVER_PID
fi
