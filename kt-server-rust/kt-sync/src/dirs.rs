...
// TODO: This could be refactored to share logic with kt-server/src/dirs.rs
...
pub fn get_server_config_path(testing: bool) -> Result<PathBuf, ()> {
    let dir = kt_server::dirs::get_config_dir()?;
...
pub fn get_sync_dir() -> Result<PathBuf, Box<dyn Error>> {
    // if KT_SYNC_DIR is set, use that
    if let Ok(dir) = std::env::var("KT_SYNC_DIR") {
        return Ok(PathBuf::from(dir));
    }
    let home_dir = home_dir().ok_or("Unable to read home_dir")?;
    Ok(home_dir.join("KomuTrackerSync"))
}
