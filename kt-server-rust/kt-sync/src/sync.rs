...
/// Based on: https://github.com/ncc-sg/kt-server/pull/50
///
/// This does not handle any direct peer interaction/connections/networking, it works as a "bring your own folder synchronizer".
///
/// It manages a sync-folder by syncing the kt-server datastore with a copy/staging datastore in the folder (one for each host).
...
impl Default for SyncSpec {
    fn default() -> Self {
        ...
        let path = Path::new("/tmp/kt-sync").to_path_buf();
        SyncSpec {
            path,
            path_db: None,
            buckets: None,
...
pub fn sync_run(
    ...
    // kt-server-rust, which is not necessarily true (kt-server-python has seperate device_id).
...
#[allow(dead_code)]
pub fn list_buckets(client: &KtClient) -> Result<(), Box<dyn Error>> {
