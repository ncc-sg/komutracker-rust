...
/// Returns the port of the local kt-server instance
pub fn get_server_port(testing: bool) -> Result<u16, Box<dyn Error>> {
    // TODO: get kt-server config more reliably
    let kt_server_conf = crate::dirs::get_server_config_path(testing)
        .map_err(|_| "Could not get kt-server config path")?;
    let fallback: u16 = if testing { 5666 } else { 5600 };
    let port = if kt_server_conf.exists() {
        let mut file = File::open(&kt_server_conf)?;
