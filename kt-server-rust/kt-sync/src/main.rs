...
//  - [x] Import local buckets and sync events from kt-server (either through API or through creating a read-only Datastore)
...
struct Opts {
    ...
    /// If not specified, use KT_SYNC_DIR env var, or default to ~/KomuTrackerSync
    #[clap(long)]
...
enum Commands {
    /// Daemon subcommand
    /// Starts kt-sync as a daemon, which will sync every 5 minutes.
    Daemon {
    ...
    /// Sync subcommand
    ///
    /// Syncs data between local kt-server and sync directory.
    /// First pulls remote buckets from the sync directory to the local kt-server.
    /// Then pushes local buckets from the kt-server to the local sync directory.
    Sync {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    info!("Started kt-sync...");

    kt_server::logging::setup_logger("kt-sync", opts.testing, verbose)?;
    ...
    if let Some(sync_dir) = opts.sync_dir {
        ...
        std::env::set_var("KT_SYNC_DIR", sync_dir);
    }
    ...
    let client = KtClient::new(&opts.host, port, "kt-sync")?;
