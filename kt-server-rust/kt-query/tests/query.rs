...
mod query_tests {
    ...
    use kt_query::DataType;
    use kt_query::QueryError;

    use kt_datastore::Datastore;

    use kt_models::Bucket;
    use kt_models::BucketMetadata;
    use kt_models::Event;
    ...
    fn test_bool() {
        ...
        match kt_query::query(&code, &interval, &ds).unwrap() {
            kt_query::DataType::Bool(b) => assert!(b),
    ...
    fn test_number() {
        ...
        match kt_query::query(&code, &interval, &ds).unwrap() {
            kt_query::DataType::Number(n) => assert_eq!(n, 1.1),
            ref data => panic!("Wrong datatype, {data:?}"),
        };
    }
    ...
    fn test_equals() {
        ...
        let res = kt_query::query(&code, &interval, &ds);
    ...
    fn test_return() {
        ...
        match kt_query::query(&code, &interval, &ds).unwrap() {
            kt_query::DataType::Number(n) => assert_eq!(n, 1.0),
            ref data => panic!("Wrong datatype, {data:?}"),
        };
        ...
        let code = String::from("return 1+1;");
        match kt_query::query(&code, &interval, &ds).unwrap() {
            kt_query::DataType::Number(n) => assert_eq!(n, 2.0),
            ref data => panic!("Wrong datatype, {data:?}"),
        };
    }
    ...
    fn test_if() {
        ...
        match kt_query::query(&code, &interval, &ds).unwrap() {
            kt_query::DataType::Number(n) => assert_eq!(n, 3.0),
            ref data => panic!("Wrong datatype, {data:?}"),
        };
        ...
        match kt_query::query(&code, &interval, &ds).unwrap() {
            kt_query::DataType::Number(n) => assert_eq!(n, 4.0),
            ref data => panic!("Wrong datatype, {data:?}"),
        };
    ...
    fn test_categorize() {
        ...
        let result: DataType = kt_query::query(&code, &interval, &ds).unwrap();
    ...
    fn test_string() {
        ...
        match kt_query::query(&code, &interval, &ds).unwrap() {
            kt_query::DataType::String(s) => assert_eq!(s, "test \" with escaped quote"),
    ...
    fn test_concat() {
        ...
        let res = kt_query::query(&code, &interval, &ds).unwrap();
        let mut v = Vec::new();
