...
mod qfunctions {
    use kt_datastore::Datastore;
    use kt_models::Event;
    use kt_transform::classify::Rule;
    ...
    pub fn find_bucket(
        ...
        let bucketname = match kt_transform::find_bucket(
            &bucket_filter,
            &hostname_filter,
            buckets.values(),
        ) {
    ...
    pub fn flood(
        ...
        // Run flood
        let mut flooded_events = kt_transform::flood(events, chrono::Duration::seconds(5));
    ...
    pub fn categorize(
        ...
        // Run categorize
        let mut flooded_events = kt_transform::classify::categorize(events, &rules);
        // Put events back into DataType::Event container
        let mut tagged_flooded_events = Vec::new();
        for event in flooded_events.drain(..) {
            tagged_flooded_events.push(DataType::Event(event));
    ...
    pub fn tag(
        ...
        // Run categorize
        let mut flooded_events = kt_transform::classify::tag(events, &rules);
        // Put events back into DataType::Event container
        let mut tagged_flooded_events = Vec::new();
        for event in flooded_events.drain(..) {
            tagged_flooded_events.push(DataType::Event(event));
    ...
    pub fn sort_by_duration(
        ...
        // Sort by duration
        let mut sorted_events = kt_transform::sort_by_duration(events);
        // Put events back into DataType::Event container
    ...
    pub fn sort_by_timestamp(
        ...
        // Sort by duration
        let mut sorted_events = kt_transform::sort_by_timestamp(events);
        // Put events back into DataType::Event container
        let mut tagged_sorted_events = Vec::new();
        for event in sorted_events.drain(..) {
            tagged_sorted_events.push(DataType::Event(event));
    ...
    pub fn merge_events_by_keys(
        ...
        let mut merged_events = kt_transform::merge_events_by_keys(events, keys);
    ...
    pub fn chunk_events_by_key(
        ...
        let mut merged_events = kt_transform::chunk_events_by_key(events, &key);
        let mut merged_tagged_events = Vec::new();
        for event in merged_events.drain(..) {
            merged_tagged_events.push(DataType::Event(event));
        }
    ...
    pub fn filter_keyvals(
        ...
        let mut filtered_events = kt_transform::filter_keyvals(events, &key, &vals);
    ...
    pub fn filter_keyvals_regex(
        ...
        let mut filtered_events = kt_transform::filter_keyvals_regex(events, &key, &regex);
        let mut filtered_tagged_events = Vec::new();
        for event in filtered_events.drain(..) {
            filtered_tagged_events.push(DataType::Event(event));
        }
    ...
    pub fn exclude_keyvals(
        ...
        let mut filtered_events = kt_transform::exclude_keyvals(events, &key, &vals);
        let mut filtered_tagged_events = Vec::new();
        for event in filtered_events.drain(..) {
            filtered_tagged_events.push(DataType::Event(event));
        }
    ...
    pub fn filter_period_intersect(
        ...
        let mut filtered_events = kt_transform::filter_period_intersect(events, filter_events);
        let mut filtered_tagged_events = Vec::new();
        for event in filtered_events.drain(..) {
            filtered_tagged_events.push(DataType::Event(event));
        }
    ...
    pub fn split_url_events(
        ...
        for mut event in events.drain(..) {
            kt_transform::split_url_event(&mut event);
    ...
    pub fn period_union(
        ...
        let mut result = kt_transform::period_union(&events1, &events2);
    ...
    pub fn union_no_overlap(
        ...
        let mut result = kt_transform::union_no_overlap(events1, events2);
        let mut result_tagged = Vec::new();
        for event in result.drain(..) {
            result_tagged.push(DataType::Event(event));
        }
