extern crate kt_client_rust;
extern crate kt_datastore;
extern crate kt_server;
extern crate chrono;
extern crate rocket;
...
mod test {
    ...
    fn wait_for_server(timeout_s: u32, client: &KtClient) {
        for i in 0.. {
            match client.get_info() {
                Ok(_) => break,
                Err(err) => {
                    if i == timeout_s - 1 {
                        panic!("Timed out starting kt-server after {timeout_s}s: {err:?}");
                    }
                }
            }
            use std::time;
    ...
    fn setup_testserver() -> rocket::Shutdown {
        use kt_server::endpoints::AssetResolver;
        use kt_server::endpoints::ServerState;

        let state = ServerState {
            datastore: Mutex::new(kt_datastore::Datastore::new_in_memory(false)),
            asset_resolver: AssetResolver::new(None),
            device_id: "test_id".to_string(),
        };
        let mut kt_config = kt_server::config::KTConfig::default();
        kt_config.port = PORT;
        let server = kt_server::endpoints::build_rocket(state, kt_config);
    ...
    #[test]
    fn test_full() {
        let clientname = "kt-client-rust-test";

        let client: KtClient =
            KtClient::new("127.0.0.1", PORT, clientname).expect("Client creation failed");
        ...
        let bucketname = format!("kt-client-rust-test_{}", client.hostname);
