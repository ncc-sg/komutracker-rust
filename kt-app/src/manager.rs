//! A process manager for KomuTracker
//!
//! Used to start, stop and manage the lifecycle modules like kt-watcher-afk and kt-watcher-window.
//! A module is a process that runs in the background and sends events to the KomuTracker server.
//!
...
impl ManagerState {
    ...
    fn update_tray_menu(&mut self) {
        ...
        let quit = MenuItem::with_id(app, "quit", "Quit KomuTracker", true, None::<&str>)
...
fn start_module_thread(
    ...
    // Special handling for kt-notify module
    if name == "kt-notify" {
        info!("Using special kt-notify handler for module: {name}");
...
fn start_notify_module_thread(
    ...
    thread::spawn(move || {
        ...
        // Always add --output-only flag for kt-notify
            ...
            .spawn()
        {
            ...
            Err(e) => {
                ...
                if error_msg.contains("No such option: --output-only") {
                    info!("kt-notify module doesn't support --output-only, falling back to default behavior");
        ...
        for line in reader.lines() {
            match line {
                Ok(line_content) => {
                    ...
                    // Debug log kt-notify output (won't show at Info level)
                    debug!("kt-notify output: {}", line_content);
                }
                Err(e) => {
                    error!("Error reading kt-notify output: {}", e);
                    break;
                }
            }
        }
...
fn discover_modules() -> BTreeMap<String, PathBuf> {
    ...
    while let Some(dir) = dirs_to_search.pop() {
        ...
        // Look for kt-* executables in this directory
        if let Ok(entries) = fs::read_dir(&dir) {
            for entry in entries.filter_map(Result::ok) {
                ...
                if let Ok(metadata) = fs::metadata(&path) {
                    ...
                    // Process only items starting with "kt-"
                    if !file_name.starts_with("kt-") {
                        continue;
                    }

                    // If it's a directory starting with "kt-", add to search stack
                    if metadata.is_dir() {
