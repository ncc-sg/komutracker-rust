use kt_server::endpoints::build_rocket;
    ...
    if *first_run {
        thread::spawn(|| {
            ...
            app.notification()
                .builder()
                .title("KomuTracker")
                .body("Welcome to KomuTracker! Click on the tray icon to launch the dashboard")
                .show()
...
impl Default for UserConfig {
    fn default() -> Self {
        ...
        if cfg!(target_os = "linux") {
            ...
            if is_wayland {
                // On Linux with Wayland, use kt-awatcher instead of separate watchers
                modules.push(ModuleEntry::Simple("kt-awatcher".to_string()));
            } else {
                // On Linux with X11 or other display servers, use traditional watchers
                modules.push(ModuleEntry::Simple("kt-watcher-afk".to_string()));
                modules.push(ModuleEntry::Simple("kt-watcher-window".to_string()));
            }
        } else {
        ...
        .setup(|app| {
            ...
            app.set_activation_policy(tauri::ActivationPolicy::Accessory);
            {
                ...
                let db_path = kt_server::dirs::db_path(testing)
                    ...
                    .unwrap()
                    .to_string();
                let device_id = kt_server::device_id::get_device_id();
                ...
                let asset_path_opt = if let Ok(path_str) = &webui_var {
                    ...
                    if asset_path.exists() {
                        ...
                        panic!("Path set via env var KT_WEBUI_DIR does not exist");
                    }
                } else {
                ...
                let server_state = kt_server::endpoints::ServerState {
                    // Even if legacy_import is set to true it is disabled on Android so
                    // it will not happen there
                    datastore: Mutex::new(kt_datastore::Datastore::new(db_path, legacy_import)),
