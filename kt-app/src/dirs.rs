//! Directory management for KomuTracker Tauri
//!
...
lazy_static! {
    static ref ANDROID_DATA_DIR: Mutex<PathBuf> =
        Mutex::new(PathBuf::from("/data/user/0/net.komutracker.app/files"));
}
...
pub fn get_discovery_paths() -> Vec<PathBuf> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if let Ok(exe_path) = std::env::current_exe() {
            ...
            if let Some(exe_dir) = exe_path.parent() {
                // In dev mode: target/debug/kt-app -> ../../../modules
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Ok(exe_path) = std::env::current_exe() {
            debug!("Current exe path: {:?}", exe_path);
            if let Some(exe_dir) = exe_path.parent() {
                // In dev mode: target/debug/kt-app.exe -> ../../../modules
                let project_modules = exe_dir
                    .parent()
                    .and_then(|p| p.parent())
                    .and_then(|p| p.parent())
        ...
        if let Ok(username) = std::env::var("USERNAME") {
            discovery_paths.push(PathBuf::from(format!(r"C:/Users/{}/kt-modules", username)));
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        discovery_paths.push(PathBuf::from(
            "/Applications/KomuTracker.app/Contents/Resources",
        ));
    }
