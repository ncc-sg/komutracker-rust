...
|-----------|----------|---------|
| `kt-server` | `/kt-server/` | Remote server with PostgreSQL, same API as kt-server-rust (Komu Tracker Server) |
| `kt-client` | `/kt-server-rust/kt-client/` | Sync module that pushes/pulls data to/from remote server (Komu Tracker Client) |
...
```mermaid
sequenceDiagram
    participant Local as kt-server (Local)
...
kt-models = { path = "kt-server-rust/kt-models" }
kt-transform = { path = "kt-server-rust/kt-transform" }  # For heartbeat logic
...
CREATE TABLE buckets (
    ...
    bucket_id VARCHAR(255) NOT NULL,       -- e.g., "kt-watcher-window_hostname"
    bucket_type VARCHAR(255) NOT NULL,     -- e.g., "currentwindow"
    client VARCHAR(255) NOT NULL,          -- e.g., "kt-watcher-window"
...
All endpoints match `kt-server-rust` API, with added authentication:
...
kt-models = { path = "../kt-models" }
kt-client-rust = { path = "../kt-client-rust" }
...
Config file location: `~/.config/komutracker/kt-client/config.toml`
...
buckets = ["*"]         # or specific: ["kt-watcher-window_host", "kt-watcher-afk_host"]

[local]
port = 5600             # Local kt-server port
```
...
## Part 3: Integration with kt-app

### 3.1 Update Workspace

Add `kt-client` to `/kt-server-rust/Cargo.toml`:
...
### 3.2 kt-app Configuration

Users can add to their `~/.config/komutracker/kt-app/config.toml`:
...
The `/api/0/query` endpoint in kt-server-rust uses a custom query language (`kt-query`). Options:

- **Option A**: Skip this endpoint initially (return "not implemented")
- **Option B**: Implement a simplified version
- **Option C**: Port `kt-query` to work with PostgreSQL
