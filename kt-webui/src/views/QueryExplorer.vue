...
div
  ...
  kt-selectable-eventview(:events="events", :event_type="event_type")
</template>
...
export default {
  ...
  data() {
    ...
    if (_.isEmpty(this.$route.query.q)) {
      queryCode = `
afk_events = query_bucket(find_bucket("kt-watcher-afk_"));
window_events = query_bucket(find_bucket("kt-watcher-window_"));
...
`.trim();
  ...
  methods: {
    query: async function () {
      ...
      // the kt-client expects an array of commands with whitespace cleaned up
      query = query.split(';').map(s => s.trim() + ';');
      const timeperiods = [moment(this.startdate).format() + '/' + moment(this.enddate).format()];

      try {
        const data = await this.$kt.query(timeperiods, query);
        this.events = data[0];
        this.error = '';
      } catch (e) {
