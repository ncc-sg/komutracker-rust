...
div
            ...
            b-button-toolbar.float-right
              b-button-group(size="sm", class="mx-1")
                ...
                b-dropdown(variant="outline-secondary", size="sm", text="More")
                  ...
                  b-dropdown-item(
                             :href="$kt.baseURL + '/api/0/buckets/' + data.item.id + '/export'",
                             :download="'kt-bucket-export-' + data.item.id + '.json'",
  ...
  b-card-group.deck
    ...
    b-card(header="Export buckets")
      b-button(:href="$kt.baseURL + '/api/0/export'",
               :download="'kt-bucket-export.json'",
               title="Export bucket to JSON",
  ...
  kt-devonly(reason="This section is still under development")
    h2.p-2 Tools

    hr

    kt-bucket-validate.p-2
...
export default {
  name: 'Buckets',
  components: {
    'kt-bucket-merge': () => import('~/components/BucketMerge.vue'),
    'kt-bucket-validate': () => import('~/components/BucketValidate.vue'),
  },
  data() {
    return {
      moment,
    ...
    import_file: async function (_new_value, _old_value) {
      if (this.import_file != null) {
        ...
        try {
          ...
          // TODO: Make kt-server report error message so it can be shown in the web-ui
          this.import_error = 'Import failed, see kt-server logs for more info';
        }
  ...
  methods: {
    ...
    importBuckets: async function (importFile) {
      ...
      return this.$kt.req.post('/0/import', formData, { headers });
    },
    ...
    async export_csv(bucketId: string) {
      ...
      link.href = url;
      link.download = `kt-events-export-${bucketId}-${new Date()
        .toISOString()
        .substring(0, 10)}.csv`;
