...
div(v-if="view")
  draggable.row(v-model="elements" handle=".handle")
    ...
    div.col-md-6.col-lg-4.p-3(v-for="el, index in elements", :key="index", :class="{'col-md-12': isVisLarge(el), 'col-lg-12': isVisLarge(el)}")
      kt-selectable-vis(:id="index" :type="el.type" :props="el.props" @onTypeChange="onTypeChange" @onRemove="onRemove" :editable="editing")
...
export default {
  ...
  methods: {
    ...
    async onTypeChange(id, type) {
      ...
      if (type === 'custom_vis') {
        const visname = prompt('Please enter the watcher name', 'kt-watcher-');
        if (!visname) return;
