...
div
  ...
  div(v-if="loading")
    span.text-muted.center.kt-loading Loading...
  div.row(v-else)
    div.col-md-12
      h3 Running
...
export default {
  ...
  mounted: async function () {
    ...
    await this.$kt.ensureBucket(this.bucket_id, 'general.stopwatch', 'unknown');
  ...
  methods: {
    startTimer: async function (label) {
      ...
      await this.$kt.heartbeat(this.bucket_id, 1, event);
      await this.getEvents();
    },
    ...
    getEvents: async function () {
      this.events = await this.$kt.getEvents(this.bucket_id, { limit: 100 });
      this.loading = false;
    },
  },
};
